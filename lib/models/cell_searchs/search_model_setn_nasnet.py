...
class NASNetworkSETN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i, cell in enumerate(self.cells):
            ...
            if self.mode == 'urs':
                s0, s1 = s1, cell.forward_urs(s0, s1)
            # elif self.mode == 'select':
            #     s0, s1 = s1, cell.forward_select(s0, s1, alphas_cpu)
            #     s0, s1 = s1, cell.forward_darts(s0, s1, ww)
            elif self.mode == 'joint':
                s0, s1 = s1, cell.forward_joint(s0, s1, ww)
            elif self.mode == 'dynamic':
                s0, s1 = s1, cell.forward_dynamic(s0, s1, self.dynamic_cell)
                s0, s1 = s1, cell.forward_darts(s0, s1, ww)
            else:
